...
class Item(models.Model):
    ...
    title = fields.CharField(
        max_length=255, index=True, validators=[MinLengthValidator(3)]
    )
    ...
    owner = fields.ForeignKeyField(
        "models.User", related_name="items", on_delete=fields.CASCADE, index=True
    )
    ...
    trade_radius_km = fields.IntField(
    ...
    async def get_all_images(self):
        ...
        return await ItemImage.filter(item_id=self.id).order_by(
    ...
    async def get_similar_items(self, limit: int = 5):
        ...
        return await Item.filter(
            category_id=self.category_id, status=ItemStatus.AVAILABLE, id__not=self.id
        ).limit(limit)
