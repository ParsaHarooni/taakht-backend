...
class ItemImage(models.Model):
    ...
    # Storage information
    storage_provider = fields.CharField(max_length=50, default="local")  # local, s3, cloudinary, etc.
    storage_path = fields.CharField(max_length=500, null=True)
    storage_metadata = fields.JSONField(default=dict)  # Additional storage-specific metadata
    ...
    async def set_as_primary(self):
        ...
        await ItemImage.filter(item_id=self.item_id, is_primary=True).update(is_primary=False)
...
ItemImage_Pydantic = pydantic_model_creator(ItemImage, name="ItemImage", exclude=("deleted_at", "processing_error"))
ItemImageIn_Pydantic = pydantic_model_creator(
    ItemImage, 
    name="ItemImageIn", 
    exclude_readonly=True,
    exclude=("deleted_at", "processing_error", "width", "height", "format", "mime_type", "file_size_bytes", "processed_at")
)
ItemImageUpdate_Pydantic = pydantic_model_creator(
    ItemImage, 
    name="ItemImageUpdate", 
    exclude_readonly=True,
    exclude=("deleted_at", "processing_error", "width", "height", "format", "mime_type", "file_size_bytes", "processed_at", "item_id")
) 