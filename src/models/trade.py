...
class Trade(models.Model):
    ...
    estimated_value_initiator = fields.DecimalField(
        max_digits=10, decimal_places=2, null=True
    )
    estimated_value_recipient = fields.DecimalField(
        max_digits=10, decimal_places=2, null=True
    )
    ...
    def is_active(self) -> bool:
        ...
        return (
            self.status in [TradeStatus.PENDING, TradeStatus.ACCEPTED]
    ...
    async def get_initiator_items(self):
        ...
        return await TradeItem.filter(
            trade_id=self.id, side="initiator"
        ).prefetch_related("item")

    async def get_recipient_items(self):
        """Get items offered by the recipient."""
        return await TradeItem.filter(
            trade_id=self.id, side="recipient"
        ).prefetch_related("item")
    ...
    async def get_counter_offers(self):
        ...
        return await Trade.filter(
            trade_type=TradeType.COUNTER_OFFER, parent_trade_id=self.id
        ).order_by("-created_at")
...
class TradeItem(models.Model):
    ...
    trade = fields.ForeignKeyField(
        "models.Trade", related_name="trade_items", on_delete=fields.CASCADE, index=True
    )
    item = fields.ForeignKeyField(
        "models.Item", related_name="trade_items", on_delete=fields.CASCADE, index=True
    )

    # Trade side (initiator or recipient)
    side = fields.CharField(
        max_length=20,
        choices=[("initiator", "Initiator"), ("recipient", "Recipient")],
        index=True,
    )
...
TradeItemIn_Pydantic = pydantic_model_creator(
    TradeItem, name="TradeItemIn", exclude_readonly=True
)
