...
class User(models.Model):
    ...
    username = fields.CharField(
        ...
        unique=True, 
        index=True,
        validators=[MinLengthValidator(3), RegexValidator(r'^[a-zA-Z0-9_]+$', 'Username can only contain letters, numbers, and underscores')]
    )
    email = fields.CharField(
        max_length=255, 
        unique=True, 
        index=True,
        validators=[RegexValidator(r'^[^@]+@[^@]+\.[^@]+$', 'Invalid email format')]
    )
...
User_Pydantic = pydantic_model_creator(User, name="User", exclude=("password_hash", "deleted_at"))
UserIn_Pydantic = pydantic_model_creator(
    User, 
    name="UserIn", 
    exclude_readonly=True,
    exclude=("password_hash", "deleted_at", "status", "is_verified", "rating", "rating_count")
)
UserUpdate_Pydantic = pydantic_model_creator(
    User, 
    name="UserUpdate", 
    exclude_readonly=True,
    exclude=("password_hash", "deleted_at", "status", "is_verified", "rating", "rating_count", "total_trades", "successful_trades")
) 