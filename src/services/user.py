...
from src.schemas.user import UserSearchRequest, UserStatsResponse, UserUpdateRequest
...
class UserService:
    ...
    async def get_user_stats(user_id: int) -> UserStatsResponse:
        ...
        # Get item statistics
        total_items = await Item.filter(
            owner_id=user_id, deleted_at__isnull=True
        ).count()
        active_items = await Item.filter(
            owner_id=user_id, status=ItemStatus.AVAILABLE, deleted_at__isnull=True
        ).count()
